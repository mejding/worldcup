...
def train_historical_model(
    ...
    metrics = {
        ...
        "log_loss": calculate_log_loss(y_test, y_proba, labels),
        "brier_score": calculate_multiclass_brier_score(y_test, y_proba, labels),
        "draw_rate_actual": float((y_test == "D").mean()),
        "draw_rate_predicted": float(y_proba[:, draw_index].mean()),
    }
