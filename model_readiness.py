...
def normalize_model_metadata(metadata: dict) -> dict:
    ...
    return {
        ...
        "training_data_start_date": _metadata_value(metadata, "training_data_start_date", "date_min"),
        "training_data_end_date": _metadata_value(metadata, "training_data_end_date", "date_max"),
        "is_demo_model": bool(metadata.get("is_demo_model", False)),
        "includes_elo_features": bool(metadata.get("includes_elo_features", "elo_diff" in metadata.get("feature_columns", []))),
        "includes_form_features": bool(metadata.get("includes_form_features", "home_points_per_match_last5" in metadata.get("feature_columns", []))),
        "includes_tournament_features": bool(metadata.get("includes_tournament_features", "is_world_cup" in metadata.get("feature_columns", []))),
        "includes_neutral_venue": bool(metadata.get("includes_neutral_venue", "neutral" in metadata.get("feature_columns", []))),
